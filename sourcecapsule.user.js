  ...
  async function createShareLink(model, debugJson, expiryDays, onProgress) {
    ...
    const created = JSON.parse(
      await gmHttp({
        ...
        // The source permalink lets the Worker keep a back-link tombstone after the
        // capsule expires, so a recipient of a dead link can still reach the post.
        body: JSON.stringify({
          expiryDays,
          sourceUrl: model.sourceUrl || '',
          title: model.heading || model.title || '',
          handle: (model.author && model.author.handle) || '',
        }),
      })
    );
